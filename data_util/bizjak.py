...
class Dataset:
    def __init__(self, args, split_path, paired=True, batch_size=None):
        ...
        self.image_size = tuple(tensor_shape)  # (256, 192, 192) in this case
    ...
    def preprocess_image(self, image):
        ...
        """
        if image.shape != self.image_size:  # Resize only if dimensions don't match
            zoom_factors = (
                self.image_size[0] / image.shape[0],
                self.image_size[1] / image.shape[1],
                self.image_size[2] / image.shape[2],
            )
            image = zoom(image, zoom_factors, order=3)  # Cubic interpolation
